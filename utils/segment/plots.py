...
@threaded
def plot_images_and_masks(images, targets, masks, paths=None, fname='images.jpg', names=None, single_special_cls=False):

    if single_special_cls: names = None
    ...
    for i in range(i + 1):
        ...
        if len(targets) > 0:
            idx = targets[:, 0] == i  # i: images id
            ti = targets[idx]  # targets for this image

            boxes = xywh2xyxy(ti[:, 2:6]).T

            # Aisu
            if not single_special_cls:
                classes = ti[:, 1].astype('int')
                labels  = ti.shape[1] == 6  # labels if no conf column
                conf    = None if labels else ti[:, 6]  # check for confidence presence (label vs pred)
            else:
                classes = ti[:, 1]
                labels  = ti.shape[1] == 6  # labels if no conf and amount column
                conf    = None if labels else ti[:, 6]  # check for confidence presence (label vs pred)
                amount  = None if labels else ti[:, 7]  # check for amount     presence (label vs pred)
            ...
            for j, box in enumerate(boxes.T.tolist()):
                if not single_special_cls:
                    ...
                    if labels or conf[j] > 0.35:  # 0.35 conf thresh
                        label = f'{cls}' if labels else f'{cls} {conf[j]:.1f}'
                        annotator.box_label(box, label, color=color, txt_color=(0,0,0))
                else:
                    cls = classes[j] if labels else amount[j]
                    color = colors(cls)
                    if labels or conf[j] > 0.35:  # 0.35 conf thresh
                        label = f'{cls:.1f}' if labels else f'{cls:.1f} (conf:{conf[j]:.1f})'
                        annotator.box_label(box, label, color=color, txt_color=(0,0,0))
            ...
            if len(masks):
                ...
                for j, box in enumerate(boxes.T.tolist()):
                    if labels or conf[j] > 0.35:  # 0.35 conf thresh
                        color = colors(amount[j]) if single_special_cls and not labels else colors(classes[j])
                        ...
                        with contextlib.suppress(Exception):
                            im[y:y + h, x:x + w, :][mask] = im[y:y + h, x:x + w, :][mask] * 0.5 + np.array(color) * 0.5
