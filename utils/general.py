...
from Shelf_Empty_Detection.utils import TryExcept
from Shelf_Empty_Detection.utils.downloads import gsutil_getsize
from Shelf_Empty_Detection.utils.metrics import box_iou, fitness
...
def check_amp(model):
    ...
    from Shelf_Empty_Detection.models.common import AutoShape, DetectMultiBackend
...
def non_max_suppression(
    ...
    nc = prediction.shape[2] - nm - 5 if not single_special_cls else prediction.shape[2] - nm - 6 # number of classes
    xc = prediction[..., 4] > conf_thres  # candidates
    ...
    for xi, x in enumerate(prediction):  # image index, image inference
        ...
        mask = x[:, mi:] if not single_special_cls else x[:, mi:-1]  # zero columns if no masks
        if single_special_cls: amount = x[:, -1:]

        # Detections matrix nx6 (xyxy, conf, cls)
        if multi_label:
            assert not single_special_cls, "multi_label not support single_special_cls"
            ...
            if not single_special_cls:
                ...
                x = torch.cat((box, conf, j.float(), amount, mask), 1)[conf.view(-1) > conf_thres]
